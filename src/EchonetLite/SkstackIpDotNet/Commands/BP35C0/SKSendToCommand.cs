...
namespace SkstackIpDotNet.Commands.BP35C0
{
    ...
    internal class SKSendToCommand(SKSendToCommand.Input input) : SimpleCommand("SKSENDTO", 20000)
    {
        ...
        internal override byte[] Arguments
        {
            get
            {
                ...
                bw.Write(Encoding.ASCII.GetBytes($"{Arg.Handle} {Arg.Ipaddr} {Arg.Port} {(char)Arg.Sec} {(char)Arg.Side} {Arg.Datalen} "));
                bw.Write(Arg.Data);
                return ms.ToArray();
            }
        }
        ...
        {
            return $"{Arg.Handle} {Arg.Ipaddr} {Arg.Port} {(char)Arg.Sec} {(char)Arg.Side} {Arg.Datalen} ({BytesConvert.ToHexString(Arg.Data)})";
        }
