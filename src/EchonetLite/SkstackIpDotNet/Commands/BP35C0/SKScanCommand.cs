...
namespace SkstackIpDotNet.Commands.BP35C0
{
    ...
    internal class SKScanCommand(SKScanCommand.Input input) : AbstractSKCommand<SKScanCommand.SKScanResponse>("SKSCAN")
    {
        ...
        public override void ReceiveHandler(object sendor, string eventRow)
        {
            ...
            {
                response.@event = new EVENT(eventRow, true);
            ...
            {
                response.eedscan = new EEDSCAN(eventRow, true);
        ...
        public class Input
        {
            ...
            /// <summary>
            /// 指定した MAC 面側で送信します。 0: B 面（B ルート側）へ送信 1: HAN 面（HAN 側）へ送信
            /// </summary>
            public Side Side { get; set; } = Side.SideB;
        }
        ...
        {
            get { return Encoding.ASCII.GetBytes($"{(Char)Arg.ScanMode} {BytesConvert.ToHexString(BitConverter.GetBytes(Arg.ChannelMask))} {Arg.Duration} {(char)Arg.Side} "); }
        }
